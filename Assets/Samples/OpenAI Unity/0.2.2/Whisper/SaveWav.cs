...
namespace Samples.Whisper
{
	public static class SaveWav
	{
        ...
        public static byte[] Save(string filename, AudioClip clip)
        {
            ...
            {
                // Escribe la cabecera vacía
                for (int i = 0; i < 44; i++) memoryStream.WriteByte(0);

                // Convierte y escribe el audio
                ConvertAndWrite(memoryStream, clip);

                // Escribe la cabecera correctamente
                WriteHeader(memoryStream, clip);

                // Retorna el audio como bytes
